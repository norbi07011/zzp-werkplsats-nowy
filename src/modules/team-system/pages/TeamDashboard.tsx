...
export const Dashboard = ({
  ...
  const WorkerFocusCard = () => {
    ...
    const activeTask =
      myTasks.find((t) => t.status === TaskStatus.IN_PROGRESS) ||
      myTasks.find((t) => t.status === TaskStatus.TODO);
      ...
      <div className="bg-white rounded-xl shadow-lg border-l-4 border-primary-600 overflow-hidden">
        <div className="bg-slate-900 p-4 flex justify-between items-center">
          <h3 className="text-white font-bold flex items-center">
            <Clock className="mr-2 text-safety-500" /> {t("todayFocus")}
          </h3>
          <span
            className={`px-2 py-0.5 rounded text-xs font-bold uppercase ${
              activeTask.priority === Priority.URGENT
        ...
        <div className="p-6">
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 bg-primary-600 text-white py-3 rounded-lg font-bold hover:bg-primary-700 transition-colors"
            >
              {t("projectDetails")}
            </button>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
              ...
              <div className="space-y-3">
                {urgentTasks.length === 0 ? (
                  <p className="text-slate-400 italic">{t("noTasks")}</p>
                ) : (
                  urgentTasks.map((task) => (
                    <div
                      key={task.id}
                      onClick={() => setActivePage("projects")}
                      className="flex items-center justify-between p-4 bg-red-50 border border-red-100 rounded-lg cursor-pointer hover:bg-red-100 transition-colors"
                    >
                      <div>
                        ...
                        <p className="text-xs text-red-600 flex items-center mt-1">
                          <Calendar size={12} className="mr-1" /> Due:{" "}
                        ...
                        <div className="flex -space-x-2">
                          {task.assignedToIds.map((id) => (
                            <img
                              ...
                              src={`https://picsum.photos/seed/${id}/50`}
                              alt="User"
                            />
                          ))}
                        </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-full bg-slate-100 rounded-full h-1.5">
                      <div
                        className="bg-safety-500 h-1.5 rounded-full"
